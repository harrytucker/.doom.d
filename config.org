...
#+end_src
**** Org LaTeX Export
Enable =mimore.cls= for exporting LaTeX reports in a nicer looking format than the
default template.
#+begin_src elisp
...
#+end_src
**** PDF Tools
Opening a =preview= window isn't as nice as opening =pdf-tools= within Emacs
instead.
#+begin_src elisp
(add-to-list 'org-file-apps '("\\.pdf\\'" . pdf-tools))
#+end_src
**** Org Roam
I sometimes link to external resources from the web, and the URLs that populate
the =org-roam-graph= take up a lot of space. So, let's remove them from the graph:
#+begin_src elisp
...
#+end_src
**** Web/Graph View
Enables a web server that can be used to interact with =org-roam= notes
graphically.
#+begin_src elisp

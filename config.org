...
If using Emacs on WSL, the alien and hybrid indexing methods for projectile will hang indefinitely when trying to cache the project files. Solution is to check if we're running WSL (via environment variable) and then setting the indexing method to native if not nil.
#+BEGIN_SRC elisp
